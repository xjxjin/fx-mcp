...
def create_app():
    ...
    app = Starlette(
        routes=[
            Mount("/", app=http_app),  # 挂载HTTP应用
        ],
        lifespan=http_app.lifespan,  # 使用MCP应用的生命周期管理
    )
...
def run_app(host="0.0.0.0", port=8080):
    ...
    print(f"正在启动PostgreSQL查询服务，HTTP和SSE端点位于: http://{host}:{port}/mcp")
